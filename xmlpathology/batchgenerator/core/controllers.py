...
class PixelLabelController(LabelController):
    ...
    def update(self, auxiliaries: Dict[str, Dict]):
        try:
            ...
            raise ValueError(
                'no \'pixel_count\' key found in auxiliaries, consider using the PixelCounter callback when using the PixelLabelController')
...
class FslSamplerController(SamplerController):
    def __init__(self,
        ...
        if len(self._dataset.labels) < n_way:
            raise ValueError(
                f'FslSampleCOntroller: number of classes ({len(self._dataset.info.labels)}) < n_way ({n_way})')
        ...
        for label, counts in self._dataset.counts_per_label.items():
            if 0 < counts < n_shots + n_queries:
                raise ValueError(
                    f'FslSampleCOntroller: {label} (n={counts})  does not have {n_shots + n_queries} samples')
