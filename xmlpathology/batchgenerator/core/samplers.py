...
class SegmentationLabelSampler(LabelSampler):
    ...
    def sample(self, image_annotation, annotation, center_x, center_y, width, height, pixel_spacing):
        ...
        # get annotations
        annotations = image_annotation.select_annotations(
...
class Sampler():
    ...
    def sample(self, image_annotation_index, annotation_index, i=None):
        ...
        center_x, center_y = self._point_sampler.sample(annotation,
        ...
        for shape_index, patch_shape in enumerate(self._input_shapes):
            for spacing_index, pixel_spacing in enumerate(self._spacings):
                # sample patch, mask, weights
                patch, mask = self._sample(image_annotation, annotation, center_x, center_y, patch_shape[0], patch_shape[1], pixel_spacing)
                ...
                # log information
                self._logger.info(
                    f'batch_index: {i} - image_annotation_index: {image_annotation_index} - annotation_index: {annotation_index} - center: {int(center_x)}, {int(center_y)} - spacing: {pixel_spacing} - patch_shape: {int(patch_shape[0])}, {int(patch_shape[1])}')
    ...
    def _sample(self, image_annotation, annotation, center_x, center_y, width, height, pixel_spacing):
        # get image path
