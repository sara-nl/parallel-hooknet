...
class BatchGenerator():
    """
    An xml batch generation class which uses loaders and classes that allcan be customized by the user.
    ...
    Args:
        ...
        annototaion_parser_loader: laoder class for parsing annotations
    ...
    def __init__(self,
                 ...
                 annotation_parser_loader: AnnotationParserLoader = AnnotationParserLoader(class_=AsapAnnotationParser,
                                                                                           sort_by='label_map'),
    ...
    def batch(self, mode):
        ...
        x_batch = [x.astype(np.float32) for x in x_batch]
        y_batch = [y.astype(np.float32) for y in y_batch]
...
class BatchGeneratorVanilla(BatchGenerator):
    ...
    def __init__(self,
        ...
        point_sampler_loader = PointSamplerLoader(class_=RandomPointSampler,
                                                  strict_point_sampling=strict_point_sampling)
...
class WSIPatchGenerator(BatchGenerator):
    ...
    def block_shaped(arr, nrows, ncols):
        ...
        return (
            arr.reshape(height // nrows, nrows, -1, ncols, channels).swapaxes(1, 2).reshape(-1, nrows, ncols, channels))
    ...
    def __init__(self,
        ...
        annotation_parser_loader = AnnotationParserLoader(WSIAnnotationParser, tile_shape=tile_shape, shift=shift[0],
                                                          spacing=spacing)
    ...
    def batch(self, mode):
        ...
        return np.array(WSIPatchGenerator.block_shaped(batch['x_batch'], self._patch_shape[0], self._patch_shape[1]))
