...
class RandomBatchGenerator:
    ...
    def _get_batch(self) -> Tuple[List, List]:
        return ([np.random.randint(0, 255, (self._batch_size, *self._input_shape)) / 255.0,
                ...
                [np.random.randint(0, self._n_classes,
                                   (self._batch_size, self._output_shape[0] * self._output_shape[1], self._n_classes))])

    def get_batch(self):
        return self._get_batch()


class Camelyon16BatckGenerator:
    def __init__(self, batch_size: int,
                 input_shape: List[int],
                 output_shape: List[int],
                 n_classes: int) -> None:
...
if __name__ == '__main__':
    batch_generator = RandomBatchGenerator(4, [299, 299, 3], [70, 70, 2], 2)
